...
<section class="relative bg-foreground text-background overflow-hidden cta-section">
	...
	<div class="relative section-pad py-20 md:py-28">
		<div class="max-w-3xl">
			...
			<div class="mt-9 flex flex-col sm:flex-row gap-4">
				<a class="button-primary" href="<?= e($primary['to']) ?>">
					<?= e($primary['label']) ?>
					<span aria-hidden="true">→</span>
				</a>
				<?php if ( isset($props['secondary']) ): ?>
				<a class="inline-flex justify-center border border-background/30 px-7 py-4 font-mono text-xs uppercase tracking-wider" href="<?= e($props['secondary']['to']) ?>">
					<?= e($props['secondary']['label']) ?>
				</a>
				<?php endif; ?>
			</div>
		</div>
