...
const server=spawn(php,['-S',`127.0.0.1:${port}`,'router.php'],{cwd:root,env:{...process.env,EDX_ENV:'production',EDX_STORAGE_PATH:storage,EDX_ADMIN_EMAIL:'qa@example.test',EDX_ADMIN_PASSWORD_HASH:hash,EDX_MAIL_TRANSPORT:'disabled'},windowsHide:true,stdio:['ignore','ignore','pipe']});
...
try{
 ...
 r=await request('/blog/prueba-migracion-local');check('Draft not public',r.status===404);
 ...
 r=await request('/blog/prueba-migracion-local');check('Published article renders server-side',r.status===200&&r.text.includes('Contenido de prueba.'));
 r=await request('/sitemap.xml');check('Sitemap updates after publish',r.text.includes('/blog/prueba-migracion-local'));
 ...
 r=await request('/sitemap.xml');check('Unpublished removed from sitemap',!r.text.includes('/blog/prueba-migracion-local'));
