...
for(const a of ['/no-existe','/react/src/App.jsx','/services/config.local.php','/content/blog.json','/var/posts.json','/tools/migrate.mjs','/node_modules/react/package.json']){const r=await fetch(base+a);if(![403,404].includes(r.status))errors.push([a,'private/missing route exposed',r.status]);}
const xml=await (await fetch(base+'/sitemap.xml')).text();const sx=cheerio.load(xml,{xmlMode:true});const sitemap=sx('loc').map((_,e)=>sx(e).text()).get();
