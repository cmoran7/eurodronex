...
- PHP 8.2 o superior con `mbstring`, `fileinfo`, `PDO_SQLite`, sesiones y JSON.
...
`index.php`, `.htaccess`, `robots.txt`, `sitemap.xml`, y las carpetas `assets`, `components`, `content`, `pages`, `services`. El router es solo para la vista previa local.
...
6. `environment = production`: habilita la indexación pública y `robots.txt`. Mantener `development` en previews o staging.
...
- Comprobar TLS, rutas limpias y denegación de acceso a `services/config.local.php`, `content/blog.json` y almacenamiento privado desde Internet.
...
- Comprobar que `/robots.txt` permite rastrear y `/sitemap.xml` contiene URLs HTTPS del dominio final. La versión dinámica es la autoridad; se actualiza con el blog.
